...
export default function HomePage() {
  ...
  const continents: CountriesProps[] = [
    { value: 16, country: "Africa", icon: <FaGlobeAfrica className=" w-16 h-16 lg:w-20 lg:h-20" /> },
    { value: 6, country: "Asia", icon: <FaGlobeAsia className=" w-16 h-16 lg:w-20 lg:h-20" /> },
    { value: 8, country: "Europe", icon: <FaGlobeEurope className=" w-16 h-16 lg:w-20 lg:h-20" /> },
    { value: 12, country: "America", icon: <FaGlobeAmericas className=" w-16 h-16 lg:w-20 lg:h-20" /> },
    { value: 10, country: "Oceania", icon: <FaEarthOceania className=" w-16 h-16 lg:w-20 lg:h-20" /> },
    { value: 13, country: "Online", icon: <CiGlobe className=" w-16 h-16 lg:w-20 lg:h-20" /> },
  ]
  ...
  return (
   ...
   {/* Hero Section */}
   {/* Hero Section */}
   <section className="relative bg-[url(/images/tiles-images.svg)]   overflow-hidden" ref={(el) => {
        if (el) {
          console.log('Hero Section Computed Style:', window.getComputedStyle(el).backgroundImage);
        }
      }}>
        <div className="container mx-auto px-4">
          <div className="w-full h-[400px] lg:h-[600px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 pt-14">
            {/* Left Column: Logo + Text (now centered relative to its grid cell) */}
            <div className="flex flex-col items-center lg:items-start"> {/* Changed items-start to items-center for centering, then lg:items-start to align left on large screens if needed, but let's center it more globally */}
              {/* Wrapper for logo and text to center them together */}
              <div className="flex flex-col items-center lg:items-start"> {/* This centers on mobile, aligns left on large screens relative to this div */}
                <Image
                  src={logo}
                  alt="WESPA Logo"
                  priority
                ...
                />
                {/* Main Title Text - Made larger, centered on mobile, left-aligned on large screens, positioned directly under logo */}
                <h1 className="text-lg md:text-xl lg:text-2xl font-medium text-gray-600 leading-relaxed max-w-xl mb-4 text-center lg:text-left"> {/* Added text-center lg:text-left */}
                ...
                {/* Link - Centered on mobile, left-aligned on large screens, aligned under text, made smaller */}
                <Link
                  href="/membership"
                  className="flex items-center gap-2 text-primary text-xs md:text-sm font-medium transition-colors text-center lg:text-left" // Added text-center lg:text-left, reduced font size to text-xs md:text-sm
                >
                  <span>Learn More</span>
                  <ChevronRight size={20} /> {/* Slightly reduced icon size for smaller text */}
                </Link>
              </div>
            </div>
            {/* Globe */}
        ...
        <section className="py-14 md:py-8">
          <div className="container-width"> {/* Using your custom container class */}
            <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {quickLinks.map((link, i) => (
                <Link href={link.href} key={i} className="block"> {/* Link wraps the entire card for clickability */}
                  <div className="text-center hover:shadow-md transition-shadow duration-200 h-[280px] w-[260px] mx-auto rounded-lg bg-white"> {/* Card-like div */}
                    <div className="p-0 h-full flex flex-col justify-between"> {/* Removed CardContent, applied styles */}
                      <div className="p-6 h-full flex flex-col justify-between"> {/* Internal padded block */}
                        {/* Icon */}
                        <div className="p-2.5 bg-primary/10 rounded-lg w-fit mx-auto mb-4"> {/* Icon container styling, centered */}
                          {link.icon}
                        </div>

                        {/* Title */}
                        <h3 className="font-semibold text-lg text-gray-900 mb-2">{link.title}</h3> {/* Match Framework title style */}

                        {/* Description */}
                        <p className="text-gray-600 text-sm mb-4 flex-grow">{link.description}</p> {/* Match Framework desc style */}

                        {/* Link - Pushed to bottom */}
                        <p className="text-primary text-sm font-medium hover:underline mt-auto">Explore</p> {/* Match Framework link style, centered */}
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      <section className="py-16 md:pt-24 md:pb-16 bg-gray-100 ">
        ...
        <section ref={ref} className="grid grid-cols-2 lg:grid-cols-3 divide-x divide-y divide-gray-300 lg:px-20">
          {continents.map((continent, idx) => (
            <div
              key={idx}
              className="flex flex-col lg:flex-row items-center justify-center gap-5 py-10 text-center"
            >
              <div className="text-5xl text-gray-900">{continent.icon}</div>
              <section className=" flex flex-col gap-3">

                <p className="text-lg md:text-2xl font-normal text-gray-800">
                  {inView ? (
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center">
        ...
        <ChevronRight
          className={`w-5 h-5 text-gray-400 transform transition-transform duration-300 ${isOpen ? "rotate-90" : ""
            }`}
        />
      </div>
